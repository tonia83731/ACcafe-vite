    ...
    <div className="grid gap-4 grid-cols-5 items-center border-b-2 border-grullo-60 px-2 py-1 hover:bg-black-5">
      ...
      <div className="col-span-2 tablet:col-span-1">
        <div className="grid gap-4 grid-cols-4">
          ...
          <button className="rounded px-2 py-1 min-w-[40px] text-xs bg-grullo-100 text-white-60 hover:text-white-100 tablet:text-sm">
            ❤
          </button>
        </div>
      </div>
...
export default function ProductList() {
  const ProductListList = dummyProductData.map((prop) => {
    return <ProductListItem prop={prop}/>
  })
  return(
    <div className="py-4">
      {ProductListList}
